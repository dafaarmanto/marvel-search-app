...
const DetailComicsList = ({ comics }) => {
  return (
    <div className="mobile:mx-6 laptop:mx-24 my-12">
      <p
        className={`pb-2 w-24 text-black font-bold mobile:text-xl laptop:text-2xl border-2 border-t-0 border-l-0 border-r-0 border-b-black uppercase ${josefin.className}`}
      >
        Comics
      </p>
            ...
            <div className="flex flex-col" key={comic.id}>
              ...
              <p className={`mt-2 text-gray-600 text-sm ${inter.className}`}>
                <a
                  ...
                  target="_blank"
                >
                  {comic.creators?.items[0]?.name === undefined
                    ? 'No authors'
